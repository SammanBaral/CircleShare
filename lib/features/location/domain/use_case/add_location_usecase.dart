import 'package:circle_share/app/usecase/usecase.dart';
import 'package:circle_share/core/error/failure.dart';
import 'package:circle_share/features/location/domain/entity/location_entity.dart';
import 'package:circle_share/features/location/domain/repository/location_repository.dart';
import 'package:dartz/dartz.dart';
import 'package:equatable/equatable.dart';

class AddLocationParams extends Equatable {
  final String locationName;

  const AddLocationParams({required this.locationName});

  @override
  List<Object?> get props => [locationName];
}

class AddLocationUseCase implements UsecaseWithParams<void, AddLocationParams> {
  final ILocationRepository locationRepository;

  AddLocationUseCase({required this.locationRepository});

  @override
  Future<Either<Failure, void>> call(AddLocationParams params) async {
    return await locationRepository
        .addLocation(LocationEntity(name: params.locationName));
  }
}
